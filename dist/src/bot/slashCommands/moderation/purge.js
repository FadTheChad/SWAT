        ...
        super("purge", client, {
            ...
            options: [
                ...
                },
                {
                    name: "match",
                    type: "STRING",
                    description: "Only purge messages that have a certain phrase string them. (Regex supported)"
                },
                {
                    name: "nomatch",
                    type: "STRING",
                    description: "Only purge messages that don't have a certain string within them. (Regex supported)"
                },
                {
                    name: "role",
                    type: "ROLE",
    ...
    async run(interaction) {
        ...
        if (!channel.viewable ||
            ...
            return interaction.reply(this.client.functions.generateErrorMessage({
                ...
                description: "I can't read messages in that channel, please make sure I have the **View Channel**, **Read Message History**, and **Manage Messages** permissions!"
            }));
        ...
        for (const message of (await channel.messages.fetch({
            ...
            if (interaction.options.getString("match")) {
                const matches = await this.client.executeRegex(new RegExp(interaction.options.getString("match"), "gi"), message.content);
                completed.push(matches !== null && matches.length > 0);
            }
            if (interaction.options.getString("nomatch")) {
                const matches = await this.client.executeRegex(new RegExp(interaction.options.getString("nomatch"), "gi"), message.content);
                completed.push(matches !== null && matches.length === 0);
            }
            if (interaction.options.getRole("role"))
                completed.push(message.member?.roles.cache.get(interaction.options.getRole("role").id));
            if (interaction.options.getBoolean("embeds") !== null)
                completed.push(interaction.options.getBoolean("embeds")
                    ? message.embeds.length > 0
                    : message.embeds.length === 0);
            if (interaction.options.getBoolean("attachments") !== null)
                completed.push(interaction.options.getBoolean("attachments")
        ...
        this.client.emit("purge", interaction.guild, interaction.member, messages, interaction.options.getString("reason"));
        let users = new Collection();
        let purgedUserCount = new Collection();
