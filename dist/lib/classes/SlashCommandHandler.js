export default class SlashCommandHandler {
    ...
    loadCommands() {
        ...
        return setTimeout(() => {
            this.client.guilds.cache.forEach((guild) => {
                try {
                    guild.commands.set(this.client.slashCommands.map((command) => {
                        return {
                            name: command.name,
                    ...
                    if (error.code === 50001)
                        this.client.logger.error("", `I encountered DiscordAPIError: Missing Access in ${guild.name} [${guild.name}] when trying to set slash commands!`);
                    else {
    ...
    async handleCommand(interaction) {
        ...
        if (!command) {
            this.client.logger.debug(`${interaction.user.tag} [${interaction.user.id}] invoked slash command ${interaction.commandName} even though it doesn't exist.`);
            const sentryId = await this.client.logger.sentry.captureWithInteraction(new Error(`Non existent command invoked`), interaction);
            this.client.guilds.cache.forEach((guild) => {
                try {
                    ...
                    if (error.code === 50001)
                        this.client.logger.error("", `I encountered DiscordAPIError: Missing Access in ${guild.name} [${guild.name}] when trying to remove non existent slash command ${interaction.commandName}!`);
                    else {
                        this.client.logger.error(error);
                        this.client.logger.sentry.captureWithExtras(error, {
                            Guild: guild.name,
